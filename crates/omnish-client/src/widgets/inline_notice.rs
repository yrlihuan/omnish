...
impl InlineNotice {
    ...
    /// 2. Scroll Up (`\x1b[1S`) — content shifts up by 1 line
    /// 3. Move up (`\x1b[1A`) — cursor on the prompt at R-1
    ...
    ///
    /// Note: when cursor is at row 0, the content at row 0 scrolls into
    /// scrollback. This is acceptable for sequential messages (startup)
    /// and irrelevant for runtime use (prompt is at the bottom).
...
mod tests {
    ...
    #[test]
    fn test_render_has_scroll_up_and_insert_line() {
        let output = InlineNotice::render("test", 80);
    ...
    /// Cursor at row 0: content scrolls into scrollback, notice appears
    /// at row 0, cursor stays at row 0. Acceptable for sequential startup
    /// messages where there is no important prompt to preserve.
    #[test]
    fn vt100_cursor_at_top_row() {
        ...
        let notice = InlineNotice::render("[omnish] connected", cols as usize);
        parser.process(notice.as_bytes());

        let screen = parser.screen();

        // Notice visible at row 0 (previous content scrolled to scrollback)
        ...
        // Cursor at row 0 (DECRC restores to saved position)
        let (cur_row, _) = screen.cursor_position();
        assert_eq!(cur_row, 0, "cursor row");
    }

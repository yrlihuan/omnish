...
/// Write the chat-mode tmux window title: `>` when no summary is available,
/// or `> <word>` when one is. The word is a single lowercase English word
/// generated by the daemon's `thread_summary` task.

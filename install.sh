...
if [[ "$LISTEN_CHOICE" == "2" ]] && [[ -n "${LISTEN_ADDR:-}" ]] && [[ -z "${OLD_VERSION:-}" ]]; then
    ...
    deploy_client() {
        ...
        # Generate a per-target client.toml inline (not scp of the daemon host's
        # copy) so each remote records the ssh target used to reach it under
        # `client_addr`. The daemon's Clients menu later reuses that target
        # for re-deploys instead of guessing from gethostname.
        # No -n on ssh here: the heredoc is ssh's stdin.
        ssh $SSH_OPTS "$target" "cat > ${remote_home}/client.toml" <<EOF || { warn "Failed to write client.toml to ${target}"; return 1; }
# omnish client configuration (generated by install.sh)
daemon_addr = "${CLIENT_DAEMON_ADDR}"
client_addr = "${target}"
EOF

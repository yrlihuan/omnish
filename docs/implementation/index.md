...
- **ClientConfig** (L9-L113)：客户端配置，含 shell 设置、守护进程地址、自动补全开关、自动更新开关、新手引导状态
...
- **update 模块** (L273-L318)：SHA-256 校验和、本地更新包缓存、版本字符串提取与 semver 比较、解压更新包并运行安装器
...
- **LlmBackend trait 与核心请求/响应类型** (L33-L106)：统一的 LLM 后端接口（complete/name/max_content_chars/list_backends/get_backend_by_name），LlmRequest 封装上下文/查询/对话历史/工具定义/思考模式，LlmResponse 封装内容块（Text/ToolUse/Thinking）/停止原因/用量统计，UseCase 区分补全/分析/对话用途
- **PromptManager（系统提示词管理）** (L107-L133)：可组合的具名片段管理器，支持从 JSON 加载、同名覆盖合并；用户可通过 chat.override.json 和 tool.override.json 覆盖
- **AnthropicBackend** (L137-L153)：Anthropic Messages API 后端，支持 Claude 模型、多轮对话、思考模式、工具调用、自动重试
...
- **Widgets 系统** (L230-L634)：交互式 UI 组件集，包含 LineEditor、LineStatus、InlineNotice、ScrollView、ChatLayout、Picker、Menu、TextView、Common
...
- **自更新系统** (L732-L798)：`/update` 透明自重启（execvp 恢复 PTY/session）、mtime 自动检测、协议级 UpdateCheck 轮询+后台下载+缓存机制
- **多轮聊天模式** (L800-L1107)：ChatSession 驱动的多轮对话循环，线程懒创建、双前缀快速恢复、线程绑定与多会话保护、空闲自动关闭、ChatLayout 统一渲染、Ctrl-C 中断、聊天历史持久化
- **Probe 系统** (L1109-L1164)：可插拔数据收集器，静态 Probe 和动态 Probe，平台信息来自客户端上报
- **主事件循环** (L1172-L1195)：poll I/O 多路复用，stdin/PTY master 监控，DSR 过滤，前缀匹配计时，OSC 133 命令跟踪
- **Polling 机制** (L1225-L1253)：渐进式间隔（1-60s）后台探测任务，差异更新 SessionUpdate，tmux 窗口标题自动更新
- **事件日志** (L1256-L1276)：全局环形缓冲区（200 条），记录 OSC 转换/补全/聊天/更新/连接/延迟等事件
- **守护进程通信** (L1278-L1326)：connect_daemon 连接/认证/协议版本检查，send_or_buffer 失败缓冲（10000 条上限）
- **显示函数** (L1344-L1370)：纯函数 ANSI 输出，分隔线/提示符/输入回显/响应渲染/幽灵文本/CJK 感知截断
- **命令分发** (L1372-L1413)：统一命令注册表，Local/Daemon 命令类型，重定向/管道解析
- **Agent 工具调用循环** (L1415-L1460)：自动工具调用/并行执行/结果反馈，ChatToolCall/ChatToolStatus/ChatToolResult 协议，redraw_tool_section 原地更新状态
- **OSC 133 与 Shell Hook** (L1627-L1647)：Bash PROMPT_COMMAND/DEBUG trap 集成，命令/CWD/readline 实时跟踪
- **架构设计** (L1590-L1685)：同步 poll I/O、TimeGapGuard 拦截策略、聊天模式两层架构（入口层+聊天层）

## omnish-daemon

守护进程主服务，管理会话、LLM、插件和定时任务。

- **DaemonServer** (L30-L44)：守护进程主服务结构，持有会话管理器、LLM 后端、定时任务管理器、对话管理器、插件管理器、工具注册表、格式化管理器、更新缓存等核心组件，提供 RPC 服务接口（Unix domain socket，最多 30 工作线程）
- **SessionManager** (L59-L79)：会话生命周期管理（注册、结束、驱逐），I/O 数据流写入，命令记录存储，补全上下文构建（弹性窗口+KV cache 预热），补全采样（pending sample 捕获与 flush），后台 JSONL 写入线程
- **ConversationManager** (L144-L171)：多轮聊天线程管理（创建、存储、加载、删除），JSONL 文件+内存双写，原始 JSON 存储格式，线程元数据 sidecar，线程恢复 UX 增强
...
- **智能体循环（Agent Loop）** (L527-L635)：多轮工具调用循环（最多 100 次迭代/600 秒超时），DaemonTool 直接执行+ClientTool 暂停/恢复转发，并行工具执行与增量状态推送，Thinking 块保留，Cancel（AtomicBool 标志）
- **聊天消息流程** (L636-L762)：ChatStart 创建/恢复线程→ChatMessage 进入智能体循环→ChatToolCall/ChatToolResult 工具转发→ChatResponse 最终响应→ChatInterrupt 中断处理
- **配置管理** (L763-L810)：ConfigSchema 基于 config_schema.toml 的 TUI 配置菜单构建器，ConfigQuery/ConfigUpdate 消息处理，动态后端选项生成
- **UpdateCache 与客户端更新** (L216-L242, L811-L824)：更新包缓存管理器（多平台包缓存、版本比较、传输锁），UpdateCheck 版本检查，UpdateRequest 流式包分发
...
- **TaskManager 与定时任务** (L850-L1050)：基于 tokio-cron-scheduler 的集中式任务管理器，内置任务：eviction、hourly_summary、daily_notes、disk_cleanup、thread_summary、auto_update
- **补全采样** (L825-L849)：pending sample 捕获→accepted 标志更新→条件写入（未接受+编辑距离+速率限制），JSONL 持久化
- **并发与锁设计** (L1640-L1658)：RwLock 分层（sessions/meta/commands 读多写少），Mutex 独占，两阶段驱逐和快照式清理避免锁争用
- **数据持久化** (L1581-L1639)：会话目录（meta.json/commands.json/stream.bin），线程文件（JSONL+meta.json），日志目录（completions/sessions/samples/daemon.log 轮转）

...
- **ClientConfig**：客户端配置，含 shell 设置、守护进程地址、新手引导状态、沙箱配置、首次 deploy 记录的 `client_addr`
...
客户端与守护进程之间的二进制通信协议，使用 bincode 序列化，帧以魔术字节 "OS" 验证，当前协议版本 v20，最低兼容版本 v14。
...
- **聊天会话**：ChatStart/Ready/End/Message/Response/Interrupt 管理聊天生命周期与线程恢复；ChatReady 含 `thread_title_word`（tmux 窗口标签）；ChatToolStatus 流式推送工具执行状态
...
- **UI 通知推送**：NoticePush（Info/Error 级别）由守护进程主动推送瞬时通知到客户端 UI，当前用于 Clients 菜单 deploy 结果反馈
- **Frame 与序列化**：帧封装请求 ID 与消息负载；消息格式为 [魔术字节(2)][长度(4)][序列化消息]
- **协议版本管理**：PROTOCOL_VERSION + MIN_COMPATIBLE_VERSION 管理兼容范围（v4-v20），编译时守卫测试检测枚举变体变化和变体索引稳定性
...
- **CompletionFormatter**：补全场景专用格式化器，通过冻结 history 区 + 追加式 recent 区优化 KV 缓存命中率；`format_sections(warmup_cutoff_ts)` 将输出拆成自包含 XML 的 `stable_prefix` / `remainder` 两块，配合 Anthropic `cache_control` 断点；支持 `live_cwd` 解决 DEBUG trap 记录旧路径问题
...
- **LlmBackend trait 与核心请求/响应类型**：统一的 LLM 后端接口（含 model_name），LlmRequest/LlmResponse 核心类型，UseCase 路由；UnavailableBackend 作为未配置时回退；TaggedMessage 新增 `cache_pos` 指定 cache_control 落在哪个 content block，便于补全请求把断点锚定在 stable_prefix
...
- **AnthropicBackend**：Anthropic Messages API 后端，支持多轮对话、思考模式（签名保留）、工具调用、提示缓存（最多 4 个 cache_control 断点，带预算强制执行与最后 N 标记保留策略）、自动重试
- **MultiBackend（多后端路由）**：根据 UseCase 路由请求；SharedLlmBackend 类型别名支持热重载；from_single() 便于测试
- **OpenAI 兼容后端**：`openai_compat.rs` 在转换 extra_messages 时将 user 消息的多段 text content blocks 合并为单个字符串，避免补全请求的三段 content（stable_prefix/remainder/query）被丢弃
...
- **提示模板（template）**：build_user_content 构建用户提示，`build_completion_parts` 构建补全提示，定期总结和线程摘要提示模板（支持多语言指令），`TITLE_WORD_PROMPT` 将线程 summary 压缩为单个小写英文单词用于 tmux 窗口标签
...
- **Probe 系统**：可插拔数据收集器，静态 Probe 和动态 Probe，平台信息来自客户端上报；`ClientAddrProbe` 上报 `ClientConfig.client_addr` 供守护进程 Clients 菜单去重与再次 deploy
- **主事件循环**：poll I/O 多路复用，stdin/PTY master 监控，DSR 过滤，前缀匹配计时，OSC 133 命令跟踪
- **Polling 机制**：渐进式间隔（1-60s）后台探测任务，差异更新 SessionUpdate，tmux 窗口标题自动更新（包括聊天模式下用 `ChatReady.thread_title_word` 设为 `> <word>`）
- **事件日志**：全局环形缓冲区（200 条），记录 OSC 转换/补全/聊天/更新/连接/延迟等事件
- **守护进程通信**：connect_daemon 连接/认证/协议版本检查，send_or_buffer 失败缓冲（10000 条上限）
- **显示函数**：纯函数 ANSI 输出，分隔线/提示符/输入回显/响应渲染/幽灵文本/CJK 感知截断；统一使用 `NEWLINE`（`\n\x1b[G`）代替 `\r\n` 规避部分终端栈（ConEmu+tmux）丢 CR 导致的错位；工具状态圆点使用 ANSI `GREEN`/`RED`，tool header 括号参数描述使用 `GRAY`，均避免 256-color 索引和 SGR 2 (faint) 的跨终端差异
...
- **OSC 133 与 Shell Hook**：Bash 与 Zsh 双 shell 支持，命令/CWD/readline 实时跟踪；hook 文件通过 tmp + rename 原子写入，避免并行安装/启动时的读写竞争
- **i18n 多语言**：编译期内嵌翻译系统，客户端默认 en，由守护进程按系统语言推送覆盖；`/help` 头部与各命令描述通过 `command.help.<slug>` 键翻译，跨 8 个语言包
- **Deploy Clients 菜单（客户端侧）**：接收守护进程推送的 `NoticePush` 显示 deploy 成功/失败，表单 `submit_label` 支持将 Done 按钮改名为 Deploy；Add client 目标接受 `host` 或 `user@host`，客户端侧预校验空/非法目标并就地提示
...
- **DaemonServer**：守护进程主服务结构，持有 SharedLlmBackend（支持热重载）、会话/对话/插件/工具/格式化管理器等，提供 RPC 服务接口；RPC 分发使用 `HandlerCtx` 捆绑共享状态并把长 match 分支抽成独立 `handle_*` 函数
- **AgentLoopState**：智能体循环状态，含 saved_up_to 增量持久化索引、用量追踪、cancel_flag 守护进程侧取消
- **SessionManager**：会话生命周期管理（注册、结束、驱逐），I/O 数据流写入，命令记录存储，补全上下文构建（弹性窗口+KV cache 预热 + `recent_frozen_until` 惰性初始化避免新命令污染 stable_prefix），补全采样，后台 JSONL 写入线程；`list_clients()` 按 `(client_addr, hostname)` 去重供 Clients 菜单
- **ConversationManager**：多轮聊天线程管理（创建、存储、加载、删除），JSONL 文件+内存双写，线程元数据（含用量统计、system_reminder 变更检测、sandbox_disabled per-thread 沙箱覆盖、title_word 缓存由 thread_summary 任务维护）
...
- **配置管理**：ConfigSchema 基于 config_schema.toml 的 TUI 配置菜单构建器，即时保存，api_key 自动转换为 api_key_cmd，支持动态占位符和客户端侧占位符展开，后端编辑/删除，沙箱规则增删改；新增 `General > Clients` 子菜单（仅 TCP-listen 模式）列出已知客户端并触发 one-shot SSH deploy，`._submit` 后缀把 Done 按钮改名为 Deploy
- **UpdateCache 与客户端更新**：更新包缓存管理器（多平台包缓存、版本比较、传输锁），UpdateCheck 版本检查，UpdateRequest 流式包分发
- **Deploy 子系统（deploy.rs + deploy.sh）**：通过 SSH 部署客户端，远端 `uname` 探测后从 `~/.omnish/updates/` 选匹配 tarball 并 `install.sh --client-only --upgrade`；`atomic_install` 规避 ETXTBSY；首次部署由远端生成 `client.toml` 并写入 `client_addr`；失败通过 stderr + exit code 传播，结果以 `NoticePush` 推送给客户端
- **SandboxRules**：沙箱许可规则模块，白名单规则
- **FileWatcher 与 ConfigWatcher**：共享文件监视基础设施，ConfigWatcher 分节发布/订阅机制，支持 LLM 后端热重载
- **TaskManager 与定时任务**：基于 tokio-cron-scheduler 的集中式任务管理器，内置任务：eviction、hourly_summary、daily_notes（基于 hourly summaries 汇总）、disk_cleanup、thread_summary（额外为线程推导单词 `title_word` 用作 tmux 标签）、auto_update；均使用 SharedLlmBackend

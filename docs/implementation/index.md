...
- **ClientConfig**：客户端配置，含 shell 设置、守护进程地址、新手引导状态
...
- **SandboxConfig / SandboxPluginConfig**：沙箱配置，含 backend 字段（bwrap/landlock/macos）和按工具名配置 permit_rules 豁免规则
...
- **config_edit 模块**：格式保留地原地更新 TOML 键值，支持点分隔路径的嵌套键值更新
...
- **配置管理**：ConfigQuery/Response/Update/UpdateResult 实现远程配置查询与修改；ConfigItem 支持 Toggle/Select/TextInput/Label 类型及 prefills 预填充数据；ConfigClient 实现守护进程到客户端的配置推送
...
- **协议版本管理**：PROTOCOL_VERSION + MIN_COMPATIBLE_VERSION 管理兼容范围（v4-v16），编译时守卫测试检测枚举变体变化和变体索引稳定性
...
- **提示模板（template）**：build_user_content 构建用户提示，build_simple_completion_content 构建补全提示（KV cache 前缀稳定性设计），定期总结和线程摘要提示模板
...
- **客户端插件系统**：ClientPluginManager 通过子进程执行工具，统一多后端沙箱（bwrap/landlock/seatbelt）、`/test lock on/off` 命令、JSON 协议；execute_tool 返回 `(content, is_error, needs_summarization)` 三元组
- **自更新系统**：`/update` 透明自重启（execvp 恢复 PTY/session）、mtime 自动检测、协议级 UpdateCheck 轮询+后台下载+缓存机制
- **多轮聊天模式**：ChatSession 驱动的多轮对话循环，线程懒创建、双前缀快速恢复、线程绑定与多会话保护、空闲自动关闭、ChatLayout 统一渲染、Ctrl-C 中断、聊天历史持久化、`/thread stats` 用量统计
...
- **OSC 133 与 Shell Hook**：Bash PROMPT_COMMAND/DEBUG trap 集成，命令/CWD/readline 实时跟踪
...
- **ConversationManager**：多轮聊天线程管理（创建、存储、加载、删除），JSONL 文件+内存双写，线程元数据（含用量统计、system_reminder 变更检测）
...
- **智能体循环（Agent Loop）**：多轮工具调用循环，DaemonTool 直接执行+ClientTool 暂停/恢复转发，persist_unsaved() 增量持久化，超时与断开时保存进度
- **聊天消息流程**：ChatStart 创建/恢复线程→ChatMessage→工具转发→ChatResponse→ChatInterrupt 中断处理
- **配置管理**：ConfigSchema 基于 config_schema.toml 的 TUI 配置菜单构建器，即时保存，api_key 自动转换为 api_key_cmd

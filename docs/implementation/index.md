...
- **omnish_dir()** (L122-L130)：获取 omnish 基础目录路径，优先级为 `$OMNISH_HOME` > `~/.omnish` > `/tmp/omnish`
- **load_client_config() / load_daemon_config()**：从配置文件或环境变量加载客户端/守护进程配置
- **config_edit 模块** (L146-L184)：格式保留地原地更新 TOML 键值，支持点分隔路径的嵌套键值更新
- **update 模块** (L278-L324)：SHA-256 校验和、本地更新包缓存、版本字符串提取与 semver 比较、解压更新包并运行安装器
- **auth 模块** (L326-L351)：认证令牌的路径获取、生成与加载，文件权限 0600
- **配置加载优先级** (L367-L390)：三级加载——环境变量指定路径 > 默认路径 > 内置默认值

## omnish-protocol

客户端与守护进程之间的二进制通信协议，使用 bincode 序列化，帧以魔术字节 "OS" 验证，当前协议版本 v12。
...
- **协议版本管理** (L430-L444)：PROTOCOL_VERSION 常量管理版本演进（v4-v12），编译时守卫测试检测枚举变体变化
...
- **工厂函数** (L268-L308)：根据配置创建后端实例；per-backend use_proxy 控制代理；effective_max_content_chars 推导
- **提示模板（template）** (L357-L375)：build_user_content 构建用户提示，build_simple_completion_content 构建补全提示（KV cache 前缀稳定性设计），定期总结和线程摘要提示模板
...
- **Widgets 系统** (L230-L668)：交互式 UI 组件集，包含 LineEditor、LineStatus、InlineNotice、ScrollView、ChatLayout、Picker、Menu（含 MenuChangeHandler 即时变更回调、失败自动回滚、Select prefills 预填充）、TextView、Common
- **Markdown 渲染** (L670-L693)：pulldown-cmark 解析，标题/粗体/代码块/列表/引用/链接/表格等 ANSI 终端样式输出
- **粘贴支持** (L695-L718)：括号粘贴模式、快速粘贴检测、多行折叠显示
- **客户端插件系统** (L719-L764)：ClientPluginManager 通过子进程执行工具，Landlock 沙箱、`/test lock on/off` 命令、JSON 协议
- **自更新系统** (L765-L837)：`/update` 透明自重启（execvp 恢复 PTY/session）、mtime 自动检测、协议级 UpdateCheck 轮询+后台下载+缓存机制
- **多轮聊天模式** (L838-L1145)：ChatSession 驱动的多轮对话循环，线程懒创建、双前缀快速恢复、线程绑定与多会话保护、空闲自动关闭、ChatLayout 统一渲染、Ctrl-C 中断、聊天历史持久化、`/thread stats` 用量统计
- **Probe 系统** (L1146-L1206)：可插拔数据收集器，静态 Probe 和动态 Probe，平台信息来自客户端上报
- **主事件循环** (L1210-L1233)：poll I/O 多路复用，stdin/PTY master 监控，DSR 过滤，前缀匹配计时，OSC 133 命令跟踪
- **Polling 机制** (L1262-L1292)：渐进式间隔（1-60s）后台探测任务，差异更新 SessionUpdate，tmux 窗口标题自动更新
- **事件日志** (L1293-L1313)：全局环形缓冲区（200 条），记录 OSC 转换/补全/聊天/更新/连接/延迟等事件
- **守护进程通信** (L1315-L1365)：connect_daemon 连接/认证/协议版本检查，send_or_buffer 失败缓冲（10000 条上限）
- **显示函数** (L1381-L1408)：纯函数 ANSI 输出，分隔线/提示符/输入回显/响应渲染/幽灵文本/CJK 感知截断
- **命令分发** (L1409-L1452)：统一命令注册表，Local/Daemon 命令类型，重定向/管道解析
- **Agent 工具调用循环** (L1453-L1499)：自动工具调用/并行执行/结果反馈，ChatToolCall/ChatToolStatus/ChatToolResult 协议，redraw_tool_section 原地更新状态
- **OSC 133 与 Shell Hook** (L1664-L1698)：Bash PROMPT_COMMAND/DEBUG trap 集成，命令/CWD/readline 实时跟踪
- **架构设计** (L1627-L1724)：同步 poll I/O、TimeGapGuard 拦截策略、聊天模式两层架构（入口层+聊天层）
...
- **AgentLoopState** (L55-L72)：智能体循环状态，含 saved_up_to 增量持久化索引、用量追踪
...
- **插件系统与内置工具** (L300-L405)：元数据+子进程分离架构，内置工具由客户端 omnish-plugin 执行，CommandQueryTool 在守护进程内执行，Landlock 沙箱，PROMPT.MD 支持
- **FormatterManager** (L422-L503)：工具结果格式化注册表，内置格式化器+ 外部格式化器子进程
- **PromptManager** (L504-L526)：可组合系统提示词片段管理，基础 chat.json + 用户 chat.override.json 覆盖/追加合并
...
- **配置管理** (L826-L888)：ConfigSchema 基于 config_schema.toml 的 TUI 配置菜单构建器，即时保存，api_key 自动转换为 api_key_cmd，Add Backend 表单含 Provider 预设选择器（从 model_presets.json 生成选项和 prefills）

...
客户端与守护进程之间的二进制通信协议，使用 bincode 序列化，帧以魔术字节 "OS" 验证，当前协议版本 v17，最低兼容版本 v14。
...
- **协议版本管理**：PROTOCOL_VERSION + MIN_COMPATIBLE_VERSION 管理兼容范围（v4-v17），编译时守卫测试检测枚举变体变化和变体索引稳定性
...
- **智能体循环（Agent Loop）**：多轮工具调用循环，DaemonTool 直接执行+ClientTool 暂停/恢复转发，persist_unsaved() 增量持久化，超时与断开时保存进度，thinking 标签处理

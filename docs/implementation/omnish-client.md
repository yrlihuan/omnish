...
- 严格前缀匹配（#439, commit cbe4fb8）：仅当 `request_input` 为空或建议以 `request_input` 开头时接受；移除 "拼接 `request_input + best.text`" 的弱启发式分支
- shell-prompt 转储防护：拒绝形如 `titan:~/docker $ python foo.py` 的建议（检测 ` $ ` 或 ` # ` 前为 `:`/`~`/`@`）
- 重复子串防护：当 `request_input` 长度 >= 10 字符且作为子串在建议中出现多次时拒绝
...
### `screen_capture::SharedScreenCapture`
嵌入式 vt100 终端模拟器的共享句柄，由 `ChatSession` 持有，供 `/test capture [N]` 命令查询：
- PTY 输出实时喂入 vt100 解析器，瞬时写入（CR 进度条、原地重绘、清屏）折叠为最终状态
- `/test capture` 返回当前可见屏幕，`/test capture N` 返回最近 N 行（含 scrollback，类似 `tmux capture-pane`）
- 流程经现有 slash-command pipe/redirect 语法（`| head/tail`、`> file`）
...
- form_mode 子菜单进入时自动在末尾追加 "Done" 按钮（`MenuItem::Button { destructive: false }`），光标到达 Button 时停止自动编辑（`form_auto_edit_active = false`）
- Button 按 Enter 确认等同于 ESC 返回：触发 handler 回调并弹出导航层级
- Button handler 直接从当前菜单项收集值（而非从累积的 changes 列表），确保未修改的默认 Select 值也被包含
- 适用于"添加新项"等表单式子菜单
- `MenuItem::Button { destructive: bool }`：destructive 标志由协议层 `ConfigItemKind::Button { style: ButtonStyle }` 决定，render 按 flag 区分 default vs destructive 样式；不再依赖路径后缀（`._delete`）或 label 文本（`"Delete"`）判定
...
### 2026-04-22 - 终端兼容性与聊天 UX 改进

**主要变更:**

1. **`/thread rename` 命令** (commit f1c08c3, #580):
   - 新增聊天模式命令 `/thread rename [name]`，设置用户指定的线程标题（优先级高于 LLM 派生的 `title_word`，summary 循环不会覆盖）；空参数清除覆盖恢复自动命名
   - `normalize_thread_name(raw)` 规范化用户输入：trim、剥离 ASCII/C1 控制字符（含 DEL）、截断到 32 个 Unicode scalar
   - 通过 `__cmd:thread rename` 守护进程命令分派，响应携带 effective_label 用于刷新 tmux 标题
2. **GNU screen 标题支持** (commit 4805e27, #581):
   - `\x1bk<name>\x1b\\` 序列虽源自 GNU screen，此前所有调用点仅依赖 `TMUX` 环境变量
   - 新增 `in_title_mux()` helper 同时检测 `TMUX` 与 `STY`，6 处调用点统一路由
3. **Shift+Enter 输入换行** (commit 9197d10, #579):
   - 聊天模式从未启用 modifyOtherKeys / CSI u，Windows Terminal、conhost、默认 ConEmu 等终端将 Shift+Enter 视为普通 CR 直接提交
   - 输入循环将括号粘贴 + modifyOtherKeys level 2 包装为单个 `KeyboardModeGuard`：`enter()` 写 `\x1b[?2004h\x1b[>4;2m`，Drop 写 `\x1b[>4;0m\x1b[?2004l`；所有退出路径（提交/Esc/Ctrl-D/空闲超时/panic）经 Drop 统一恢复

### 2026-04-23 - 聊天 Markdown 边界、tmux 标题、/resume 跨主机、补全防御、插件包同步（多 PR）

**主要变更:**

1. **Markdown NEWLINE 边界样式自洽** (commit 0de8787, #582):
   - `markdown::render` 改写为栈式 Renderer，在每个 NEWLINE 前 RESET 所有活动样式，之后重新应用栈，保证 NEWLINE 分隔的每行自洽
   - inline code、规则、列表 bullet 使用 scoped helper 保存/恢复外层栈，inline code 不再丢失外层 BOLD/ITALIC
   - 解决 live view 与 ScrollView expanded view 间样式渲染不一致（同一响应在两种视图下显示不同颜色）的问题
2. **聊天入口 tmux 标题** (commit 6524f66 / 32fd03f, #585 part 1):
   - 进入聊天显示 `> chat`（无 summary 词时），有 thread summary word 时切换为 `> <word>`
3. **`/resume` 默认按当前主机、新增 `/resume all`** (commit a350816, #583/#584):
   - 之前 `/resume` 显示共享 daemon 的所有主机 20 条线程混合且有上限。现在 `/resume` 仅当前主机不设上限；`/resume all` 跨所有主机不设上限；`/resume N` 按上次 fetch 缓存按序号选择
   - `/thread list` 仍保持 20 条默认（文本输出）
4. **补全建议防御** (commit cbe4fb8, #439):
   - 严格前缀匹配：仅当 `request_input` 为空或 `best.text` 以 `request_input` 开头时接受（移除以弱共同前缀启发式拼接 `request_input + best.text` 的分支）
   - shell-prompt 转储防护：拒绝形如 `titan:~/docker $ python foo.py` 的建议（检测 ` $ ` 或 ` # ` 前为 `:`/`~`/`@`）
   - 重复防护：当 `request_input` 长度 >= 10 字符且作为子串在建议中出现多次时拒绝
5. **客户端插件包镜像同步** (commit c7bc615 / 1d33a36, #588):
   - `sync_plugin_bundle` 与 `UpdateCheck` 同节奏运行；客户端 `current_checksum` 由 `omnish_common::plugin_bundle::build_bundle` 在 `spawn_blocking` 中现场计算（基于本地 `plugins/` 状态），不再读取 `.bundle_checksum` 持久文件
   - `download_plugin_bundle` 通过 `UpdateChunk` 流接收，`extract_and_mirror_plugins` 以镜像语义合并：archive 中文件覆盖目的端，目的端独有文件（含任意层级 dotfile 如用户编辑的 `tool.override.json`）保留
   - 同主机部署（默认 Unix socket 或 TCP loopback）客户端 checksum 与 daemon 自然相等，poll 变成 no-op，消除 daemon 重建后自我变更的竞态
6. **NoticePush 定向通知** (commit c000c66 配套客户端, #589):
   - 新增 `pending_notices` 模块：`expect(handler_name, ttl=90s)` 在提交前注册期望；接收 `NoticePush` 时按 `kind` 标签 `consume` 决定显示（仅初始者看到自身触发的通知，其他客户端静默丢弃）；未携带 `kind` 的 legacy 广播一律显示

### 2026-04-25 - 直接 `/model` 切换、工具结果渲染修复

**主要变更:**

1. **`/model <name>` 直接切换** (commit 46a635b):
   - 裸 `/model` 仍打开 picker；`/model <name>` 在 daemon 后端列表中按名称查找并应用（已存在线程发送 model-only `ChatMessage`，新线程暂存 `pending_model`）；找不到时列出可用名称
   - `fetch_models` / `apply_model_selection` 在 picker 和直接路径间共享
2. **工具结果渲染修复** (commit accf2c2):
   - 工具结果区段使用 `redraw_tool_section()` 替代手写游标算术，统一通过原地重绘维护多行工具区段

### 2026-04-27 - macOS 主机名归一化（#600）

**主要变更:**

1. **`.local` 后缀容差** (commit f5e06eb):
   - macOS `gethostname()` 可能返回短名或 `.local` mDNS 形态（视系统 scutil HostName 设置而异），导致 `check_resume_mismatch` 将同一机器误判为跨主机
   - 比较前对两侧各去掉一次尾部 `.local`（大小写不敏感）

### 2026-05-06 - 屏幕捕获（#602）与 Button 配置项渲染

**主要变更:**

1. **嵌入式 vt100 屏幕捕获** (commit 0b59240, #602):
   - 新增 `screen_capture` 模块：vt100 解析器镜像 PTY 输出，瞬时写入（CR 进度条、原地重绘、清屏）折叠为最终状态
   - 新增 `/test capture [N]` 命令：返回可见屏幕或最近 N 行（类似 `tmux capture-pane`），复用 slash-command pipe/redirect 语法（`| head/tail`、`> file`）
   - 后续 commit 81ca5d5 升级 vt100 到 0.16 修复 debug 构建中的溢出
2. **Button kind 渲染** (commit 3f3e2bf):
   - `ConfigItemKind::Button { style }` 替代基于 path 后缀（`._delete`）和 label 字符串（`label == "Delete"`）的渲染 hack
   - `build_menu_tree` 仅按 kind 分派，`MenuItem::Button` 新增 `destructive: bool` 字段；auto-append 表单 submit 设 `destructive: false`；render 直接使用 flag

### 2026-05-06 - `erase_ghost_text(wrapped=true)` 光标列恢复（#599）

**主要变更:**

1. **wrap 情况下的光标列还原** (commit 606d28a):
   - 当 ghost text 跨越行边界时，erase 后需要返回到原光标列，而非简单回退到行首

...
- `NoticePush`: 守护进程 -> 客户端推送临时 UI 通知（`NoticeLevel::Info`/`Error`），当前用于 Clients 菜单的 deploy 结果反馈
...
协议版本通过`PROTOCOL_VERSION`（当前值20）和`MIN_COMPATIBLE_VERSION`（当前值14）两个常量定义。服务器接受对端版本 >= MIN_COMPATIBLE_VERSION 的连接。`versions_compatible(my_min, peer_version)` 函数封装此判断。

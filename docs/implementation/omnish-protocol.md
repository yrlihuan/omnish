...
omnish-protocol 定义了客户端和守护进程之间交换的消息类型，使用bincode进行二进制序列化。协议包含一个简单的帧格式，每个帧包含请求ID和消息负载，使用魔术字节"OS"(0x4F 0x53)进行验证。当前协议版本为v8（`PROTOCOL_VERSION = 8`），客户端和服务器在认证阶段进行版本协商。
...
### `AuthOk`
认证成功响应，服务器在令牌验证成功时返回：
- `protocol_version`: 服务器协议版本号

**用途：** 客户端可根据返回的服务器版本号检测协议版本不匹配，版本不匹配时仅产生warning而不拒绝连接。

### `AuthFailed`
认证失败响应，服务器在令牌验证失败时返回此消息后关闭连接。无字段。
...
协议版本通过`PROTOCOL_VERSION`常量定义（当前值为8），用于客户端和服务器之间的版本协商。

...
**用途:**
- `PromptStart`: 开始新命令
- `CommandStart`: 标记命令已输入，存储`command`→`osc_command_line`、`cwd`→`osc_cwd`、`original`→`osc_original_input`
- `OutputStart`: 开始收集输出
- `CommandEnd`: 完成命令并包含退出码

### 命令行解析优先级（`finalize_command`）

`finalize_command()` 确定最终 `command_line` 的优先级：
...
各字段可选，解析器按分号分隔，识别`cwd:`和`orig:`前缀。例如：

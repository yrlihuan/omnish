...
- `plugin_bundler`: `Arc<PluginBundler>` - 插件包打包器（缓存最新 checksum），供 `PluginSyncCheck` 处理与 `PluginBundleTask` 共享
- `clients_history`: `Arc<ClientsHistory>` - 持久化的客户端连接历史（`$omnish_dir/clients.json`），deploy 菜单的真值源
...
- `generation`: `u64` - 启动时从 `ConversationManager` 捕获的 per-thread 单调 generation token；`is_superseded(cm)` 在每个磁盘写入点与客户端发送点检查，若 token 已过期则静默 bail（中断 + 新消息竞态时旧 loop 不再覆盖新 loop 的 tool_use_id）
...
- **目录事件自动激活/卸载（#593）**：`FileWatcher` 在 `plugins_dir` 上同时监听 `IN_CREATE`/`IN_DELETE`/`IN_MOVED_FROM`，新增 `PluginManager::rescan()` 调和 on-disk 子目录与内存插件列表，按需添加或移除 `ToolRegistry` 条目；`reload_plugins` 折入 `rescan` 后 daemon.toml 路径与文件系统路径汇聚到单一调和例程，消除两个 watcher 任务间的顺序依赖；`rescan` 在 `read_dir` 失败时早退避免瞬时不可读时大规模卸载已加载插件；为处理 `mkdir 然后 cp` 的两步模式（inotify on plugins_dir 非递归），rescan 为每个插件子目录额外注册 watch
...
- **线程元数据**（`ThreadMeta`）：每个线程有对应的 `.meta.json` sidecar 文件，包含：
  ...
  - `title_override`: `Option<String>` - 用户通过 `/thread rename <name>` 设置的标题覆盖（优先级高于 `title_word`），`effective_title_label()` 返回有效标签；空参数清除恢复自动命名
...
- **`update_meta(tid, |m| ...)` 原子读改写（#587）**：避免 `thread_summary` 任务在 ~1s 异步 LLM 调用期间加载的 ThreadMeta 副本被覆盖时清掉并发写入（如 `/thread sandbox off` 同时修改 `sandbox_disabled`）；helper 在持有 meta 锁的临界区内重新从磁盘加载、应用闭包补丁、写回。`set_sandbox_disabled` / `set_title_override` / `thread_summary` 都通过此 helper，仅修改自身字段而不覆盖并发字段
- **per-thread generation token**：`ConversationManager` 维护一个 `Mutex<HashMap<thread_id, u64>>`，每次 `handle_chat_message` 入口自增；`AgentLoopState` 在启动时捕获当前值。`is_superseded(cm)` 在每次磁盘写入与客户端发送点检查，若 token 已过期则静默返回，避免中断+新消息引发并发 agent loop 时旧 loop 注入与新 loop 的合成 tool_result 重复的 tool_use_id
- **`merge_user_query_into_tail`**：当新 `ChatMessage` 到达且尾部为 user-role 时，将 query 合并到同一条 user 消息（支持 tool_result-only / tool_result+marker+text / 纯 text 三种尾部形状），避免两条连续 user 角色违反 Anthropic 契约；`merge_interrupt_markers` 启动时 sweep 历史文件统一格式
- **conversations 子命令 token**：`conversations` 协议命令支持 `all` / `current_host` / 数字 limit 组合 token；`current_host` 时按线程 `host` 字段过滤，由 `/resume` 默认调用
...
### `PluginBundler`（`crates/omnish-daemon/src/plugin_bundle.rs`）
插件包打包器，对 `~/.omnish/plugins/` 调用 `omnish_common::plugin_bundle::build_bundle` 生成 tarball，供客户端镜像同步消费。守护进程内为带缓存的状态封装层（实际打包逻辑在 omnish-common，daemon 与客户端共享）。

**核心字段：**
- `plugins_dir`: `PathBuf` - 待打包目录
- `current`: `RwLock<Option<Bundle>>` - 最新一次打包结果缓存
- `rebuild_lock`: `Mutex<()>` - 序列化 `rebuild()` 调用，避免并行打包
- `last_rebuild_ms`: `AtomicU64` - 上次 rebuild 时间戳，500ms 合并窗口去重 bursty 调用

**主要方法：**
- `rebuild()` - 异步 rebuild 单一入口（被 `PluginBundleTask`、startup、`PluginSyncCheck` handler 共同调用）；持有 `rebuild_lock` 后若 `last_rebuild_ms` 不到 500ms 则直接返回缓存
- `snapshot()` - 读取当前缓存的 Bundle
- `checksum()` - 返回当前 SHA-256（缺失时返回空串）

**设计目的：**
- 单一 rebuild 入口避免 scheduled task 与 handler 并发互相覆盖
- 500ms 合并窗口合并 bursty 客户端 poll，避免重复打包
- 客户端 checksum 与 daemon 缓存不一致时，`PluginSyncCheck` handler 在回答前先 rebuild，消除 5 分钟陈旧缓存导致客户端镜像下载倒退本地编辑的竞态（#588 hardening）

### `ClientsHistory`（`crates/omnish-daemon/src/clients_history.rs`）
持久化的客户端连接历史，使 `config -> general -> clients` deploy 菜单在守护进程重启或主机被驱逐后仍能显示曾连接过的目标。

**核心字段：**
- `path`: `PathBuf` - 持久化文件路径（`$omnish_dir/clients.json`）
- `entries`: `RwLock<HashMap<(String, String), ClientHistoryEntry>>` - 按 `(deploy_addr, hostname)` 键控；每条记录 `first_seen` / `last_seen`

**主要方法：**
- `load_existing(path)` - 启动加载并执行 90 天剪枝（删除超过 90 天未见的条目），原子写回
- `touch(deploy_addr, hostname)` - 每次 `SessionManager::register()` 调用，更新 `last_seen` 或插入新条目
- `forget_addr(deploy_addr)` - 根据 deploy_addr 删除所有同地址条目（per-host Forget button 触发，路径仅编码 deploy_addr 实现一键忘记）
- `prune_older_than(days)` - 剪枝 N 天前 last_seen 的条目

**list_clients 集成：**
- `SessionManager::list_clients()` 以 `ClientsHistory` 持久条目为真值源，覆盖以内存中非 ended 会话的 `is_active=true` 标志
- 守护进程重启或会话目录 48h 清理不再清空 deploy 菜单
...
- **中断后保留 thinking 契约**：`ChatInterrupt` 不再写合成 `<event>user interrupted</event>` assistant marker（在 extended-thinking 模式下，DeepSeek Anthropic-compat 端点会拒绝缺 thinking 块的合成 assistant 消息）。改为仅持久化 `user[tool_result*]` 快照；下一条 `ChatMessage` 到达时通过 `merge_user_query_into_tail` 把 interrupt marker 与新 query 作为两个 text block 追加到同一条 user 消息（参考 Claude Code 格式）。`reconstruct_history` 渲染合并消息时把 marker 跳过；`handle_chat_interrupt` 的 "loop already finished" 分支变为 no-op
- **per-thread generation 失效检查**：`handle_chat_interrupt` 与 `handle_tool_result` 在 state generation 过期时短路返回，延迟到达的 tool_result 不再唤醒已废弃的 loop
...
- **定向通知**：`spawn_deploy` 携带 `kind`（`"add_client"` 或 `"deploy_client"`），`NoticePush` 仅在初始者注册了同 kind 期望时显示，避免单一客户端触发的部署广播打扰所有连接的客户端
- **持久化历史**：`ClientsHistory.touch()` 在每次 `register()` 路径调用，deploy 菜单从持久条目读取（叠加内存活跃状态），守护进程重启或 48h 会话目录清理后菜单仍记忆所有曾连接的主机；每个目标自带 destructive `Forget` 按钮，路径仅编码 `deploy_addr` 实现一键忘记同地址下所有 hostname

### Plugin Install 子系统（`plugin_install.rs`）

`Plugins` 菜单顶部 `Install plugin` 表单触发的异步安装：
- 用户输入 URL（http/https）或本地 `tar.gz` 路径并提交，守护进程异步下载、校验、合并到 `~/.omnish/plugins/<name>/`
- **包契约**：恰好一个顶级目录（点文件忽略），该目录必须含 `tool.json`；10 MiB 压缩大小上限
- **合并语义**：rsync 式--archive 中的文件覆盖目的端，但目的端独有文件（如用户编辑的 `tool.override.json`）保留
- 结果通过 `NoticePush` 反馈（同样按 `kind="install_plugin"` 定向给发起者）
- 注意：phase 1 守护进程尚不会自动 hot-discover 新安装的插件（已由 #593 在后续 commit 实现），见 PluginManager 目录事件自动激活段落

### ConfigSideEffects

`apply_config_changes` 返回值，承载 TOML 写入之外的副作用任务列表，由 server 主循环在写入成功后扇出执行：
- `deploy_targets`: `Vec<DeployTarget>` - `(handler_name, target)` 用于 deploy/add_client 触发的 `spawn_deploy`
- `plugin_installs`: `Vec<PluginInstallTarget>` - 用于 plugin 安装任务的 URL 或本地路径
- `client_forgets`: `Vec<String>` - 用于从 `ClientsHistory` 清除条目的 `deploy_addr` 列表
...
#### 7. `plugin_bundle` - 插件包定期重打（#588）

**执行周期：** 默认 `*/5 * * * *`（每 5 分钟）
**功能：** 对 `~/.omnish/plugins/` 重新打包并刷新 `PluginBundler` 缓存的 checksum，供客户端镜像同步消费
**机制：**
- 转化为正式 `ScheduledTask`（替代 ad-hoc `tokio::spawn` 轮询循环），加入与 eviction/auto_update/thread_summary 等同一生命周期管理
- 重建闭包对比新 checksum 与捕获的 `Mutex<Option<String>>` prev_checksum，仅在实际变化时记录 info 日志
- cron 最小粒度 1 分钟，与该任务设计的检测延迟预算匹配
- 启动时仍保留一次 `rebuild()` 调用以确保首次客户端 poll 时就有可用 bundle
- 用户可在 `daemon.toml` 中调优或禁用：
  ```toml
  [tasks.plugin_bundle]
  enabled = true
  schedule = '*/5 * * * *'
  ```
**实现：** 通过 `PluginBundleTask` 实现 `ScheduledTask` trait（`crates/omnish-daemon/src/plugin_bundle_task.rs`），通过 `DaemonContext.plugin_bundler` 访问 `PluginBundler`；与 `PluginSyncCheck` handler 共享同一 `rebuild()` 入口（mutex 串行化 + 500ms 合并窗口）消除 scheduled vs handler 间的陈旧缓存竞态
...
### 2026-04-22 - `/thread rename` 支持

**主要变更:**

1. **ThreadMeta 新增 `title_override`** (commit f1c08c3, #580):
   - 用户通过 `/thread rename <name>` 设置永久标题，优先级高于 LLM 派生的 `title_word`，summary 循环不会清除
   - 新增 `effective_title_label()` helper 返回有效标签
   - `build_resumed_chat_ready` 与 `__cmd:thread rename` handler 使用 effective_label 让 tmux 重命名后立即生效
   - 通过 `update_meta` helper 原子写入

### 2026-04-23 - 插件包同步、补全 JSON 严格化、thread_summary 元数据安全（多 PR）

**主要变更:**

1. **守护进程侧插件包打包** (commit 5fd9874 / 2a1a0c6 / 523d786 / c2a5526 / 1d33a36, #588):
   - 新增 `plugin_bundle` 模块，构建 `~/.omnish/plugins/` 的确定性 gzip tar；后续将打包逻辑迁移到 omnish-common，`PluginBundler` 改为带缓存的状态封装
   - 文件权限归一化（目录与可执行 0755，数据 0644）、mtime 固定，相同输入产生相同 checksum
   - 协议新增 `PluginSyncCheck`/`PluginSyncInfo`/`PluginSyncRequest`，复用 `UpdateChunk` 流式传输 tarball
   - `server.rs` 抽出 `stream_chunks` 共享 helper（generic over `AsyncRead`），既被 `handle_update_request` 文件路径也被 `handle_plugin_sync_request` 内存字节 (`&[u8]`) 复用，统一 chunk 发射循环、`send_update_error`、done 标记
   - 重打从 ad-hoc spawn 循环迁移为 `PluginBundleTask` 正式 `ScheduledTask`
   - **陈旧缓存竞态修复**：`PluginSyncCheck` handler 在客户端 checksum 与缓存不一致时先 `rebuild` 再回答；单一 `rebuild()` 入口由 `Mutex<()>` 串行化 + 500ms 合并窗口去重，消除 daemon 重建 5 分钟内客户端 poll 触发对手户本地编辑的镜像下载倒退
2. **补全 JSON 严格化** (commit 9c89b6f, #586):
   - 补全响应解析丢弃 plaintext fallback，只接受两种文档化 JSON 数组形态；错误形 JSON 对象（如 `{"error": ..., "command": "exit"}`）不再被解读为建议
3. **`thread_summary` 元数据并发安全** (commit 0dae3a8, #587):
   - 加入 `ConversationManager::update_meta(tid, |m| ...)` 原子读改写 helper，避免 `thread_summary` 在 ~1s LLM 调用期间的 stale clone 覆盖并发 `/thread sandbox off` 等修改
   - `thread_summary` 的 save 路径改为仅 patch 自身字段（`summary`、`summary_rounds`、`title_word`）于最新 disk meta；`set_sandbox_disabled` / `set_title_override` 同步迁移
4. **`/resume` 跨主机与上限改造** (commit a350816, #583/#584):
   - `conversations` 子命令支持组合 token：`all`、`current_host`、数字 limit；存在 `current_host` 时解析会话 hostname 并按 `ThreadMeta.host` 过滤
   - `/resume` 默认按当前主机过滤无上限；`/resume all` 跨主机无上限；`/resume N` 索引上次 fetch 缓存

### 2026-04-23 至 04-24 - URL/本地包安装插件、deploy 通知定向、插件子目录热增删（多 PR）

**主要变更:**

1. **URL/本地 tar.gz 安装插件** (commit c000c66, #589):
   - `Plugins` 菜单顶部 `Install plugin` 表单：用户输入 URL（http/https）或本地路径，守护进程异步下载、校验、合并到 `~/.omnish/plugins/<name>/` 并通过 `NoticePush` 反馈
   - 包契约：恰好一个顶级目录（dotfile 忽略），含 `tool.json`；10 MiB 压缩大小上限；合并为 rsync 风格保留目的端独有文件
   - 协议 `NoticePush` 新增 `kind: Option<String>` 标签；客户端 `pending_notices::expect(handler_name, 90s)` 注册期望后按 kind consume
   - `apply_config_changes` 返回 `ConfigSideEffects { deploy_targets, plugin_installs }`，server 主循环在 TOML 写入后扇出 deploy 与 install 任务
   - 协议 v22；MIN_COMPATIBLE_VERSION 保持 14，旧客户端会静默丢弃新 daemon 的 NoticePush 帧
2. **Deploy 通知定向** (commit cac6dcf):
   - `deploy.rs` 不再无差别广播 `NoticePush`，`spawn_deploy` 通过 `ConfigSideEffects.deploy_targets` 携带的 handler 名（`add_client` / `deploy_client`）作为 kind 传出，与初始者的 `expect` 匹配
3. **插件子目录热增删自动激活** (commit f005248, #593):
   - `FileWatcher` 目录监视新增 `IN_CREATE`/`IN_DELETE`/`IN_MOVED_FROM`，新增 `PluginManager::rescan` 调和 on-disk 子目录与内存插件列表
   - `reload_plugins` 折入 `rescan`，统一 daemon.toml 事件路径与文件系统事件路径
   - `rescan` 早退保护：`read_dir` 失败时不批量卸载已加载插件；新插件 slot 在工具注册前 push，避免 `plugin_idx` 依赖 insert/push 顺序
   - `file_watcher` 模块移至 library，使 `PluginManager` 可引用

### 2026-04-25 - per-thread generation token、中断 thinking 契约保留

**主要变更:**

1. **per-thread generation token** (commit 12ab6c8):
   - 用户中断 tool 执行后立即发送新 `ChatMessage`，`ChatToolResult` / `ChatInterrupt` / `ChatMessage` 各自在独立 tokio 任务（每消息 spawn），旧 agent loop 与新 `handle_chat_message` 竞速同一线程文件，旧 loop 的 cancelled 路径写入真实 tool_result 而新 loop 已注入合成 tool_result，产生重复 `tool_use_id` API 拒绝
   - `ConversationManager` 维护 per-thread 单调 generation token，每次 `ChatMessage` 自增；`AgentLoopState` 启动时捕获值；每个磁盘写或客户端发送点先 `is_superseded`，过期则静默 bail
   - `handle_chat_interrupt` 与 `handle_tool_result` 在 state generation 过期时短路返回
   - 替代方案"按线程互斥"的劣势：会强迫用户等待旧 LLM 调用完成才能开始新消息，违背 interrupt 的本意
2. **中断保留 thinking 契约** (commit 05f38ae):
   - Ctrl-C 期间，守护进程不再注入合成 `<event>user interrupted</event>` assistant 消息（在 extended-thinking 模式下，DeepSeek Anthropic-compat 拒绝缺 thinking 块的合成消息一旦 cache_control 落上）
   - 仅持久化 `user[tool_result*]` 快照；下一条 `ChatMessage` 通过 `merge_interrupt_markers` 与 `merge_user_query_into_tail` 把 interrupt marker 与新 query 作为两个 text block 追加到同一条 user 消息（Claude Code 格式）
   - `reconstruct_history` 渲染合并消息时跳过 marker；启动时 sweep 历史文件统一格式；`handle_chat_interrupt` 的 "loop already finished" 分支变为 no-op
3. **合并谓词扩展** (commit 21c2bfe):
   - 用户连续发送两次 "继续" 时，尾部已是 `user[tool_result, marker, "继续"]`，原狭窄的 "tool_result-only" 谓词失败导致追加新 user 消息，违反 Anthropic 双 user 角色禁令
   - 改用宽 "尾部是 user-role" 谓词路由到 `merge_user_query_into_tail`，处理三种尾部形状（tool_result-only / tool_result+marker+text / 纯 text）；遗留 `is_user_tool_result_only_public` 移除

### 2026-05-06 - Button kind 与 ButtonStyle、clients 历史持久化（多 PR）

**主要变更:**

1. **Button kind 与 ButtonStyle** (commit 3f3e2bf, #604 配套):
   - 协议 `ConfigItemKind::Button { style: ButtonStyle }` 替代基于路径后缀（`._delete`）与 label 字符串的 hack；守护进程在 schema 中决定 destructive vs default，客户端严格按 kind 渲染
   - daemon: backend 子菜单 `_delete` -> `Button { Destructive }`；deploy 子菜单 `_submit` -> `Button { Default }`（form-bottom layout 仍按 `._submit` 后缀分派）
   - `._submit` / `._delete` 路径后缀对 daemon handler 分派仍有意义，仅不再参与客户端渲染决策
   - PROTOCOL_VERSION 22 -> 23，MIN_COMPATIBLE_VERSION 同步抬升至 23
2. **clients 历史持久化** (commit c6b7265, #604):
   - 新增 `ClientsHistory` 模块（`clients_history.rs`）按 `(deploy_addr, hostname)` 键控，记录 `first_seen` / `last_seen`，原子写 `$omnish_dir/clients.json`
   - `SessionManager` 持有 history，每次 `register()`（fast / double-check / slow 路径）调用 `touch()`；新增 `forget_client_addr()` 与 `prune_clients_history()`
   - `list_clients()` 以持久条目为真值，叠加内存非 ended 会话的 `is_active=true`，守护进程重启后 deploy 菜单不再丢失曾连过的主机
   - 启动时 90 天剪枝（在 `load_existing` 中）控制文件大小，3 个月未触达的主机自动落榜
   - per-host `Forget` 按钮（destructive Button）路径仅编码 `deploy_addr`，一次清除同地址下所有 hostname；通过新 `forget_client` 动态 handler 分派
   - `ConfigSideEffects` 新增 `client_forgets: Vec<String>`，`handle_config_update` 在 TOML 写后应用
3. **`/resume all` 跨主机列出显示主机** (commit 8acf519, #606):
   - `host_filter` 为 None 时在时间与轮次列间插入 `@<host>` 列，单主机模式保持原格式
   - `short_host()` 去掉 macOS `.local` mDNS 后缀（与 #600 主机归一化一致），12 字符封顶；遗留无主机记录的线程显示 `?` 占位

### 2026-05-11 - `/thread stats [N]` 路由修复（#608）

**主要变更:**

1. **handle_builtin_command 分派修复** (commit 7ed5d05):
   - 之前 `"conversations stats"` 精确匹配，非空 tail 退回通用 `"conversations"` 分支，导致 `/thread stats N` 渲染 `/thread list N` 输出
   - 改为先匹配 `"conversations stats[ <args>]"`，解析可选 limit（默认 10），应用到 `format_thread_stats` 的多线程回退路径；active-thread 路径不变
   - 新增 `test_thread_command.sh` 覆盖三种显示变体
